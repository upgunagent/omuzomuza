...
// Dynamic import for the PDF Preview Modal to avoid SSR issues
const CvPreviewModal = dynamic(() => import("@/components/features/cv/CvPreviewModal"), {
    ssr: false,
    loading: () => <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50"><Loader2 className="w-8 h-8 animate-spin text-white" /></div>
});
...
export default function CandidateProfilePage() {
    ...
    // Update preview when candidate data loads
    useEffect(() => {
    ...
    async function fetchProfile() {
        ...
        if (user) {
            ...
            if (data) {
                ...
                setCandidate({ id: user.id, email: user.email });
            }
        } else {
            // Mock data for dev
            setCandidate({
                ...
                summary: 'This is a mock profile.'
            });
        }
        setLoading(false);
    }
    ...
    async function handlePreviewCv() {
        ...
        try {
            ...
            const [
                ...
                supabase.from('resume_education').select('*').eq('candidate_id', user.id).order('start_date', { ascending: false }),
                supabase.from('resume_experience').select('*').eq('candidate_id', user.id).order('start_date', { ascending: false }),
                supabase.from('resume_skill').select('*').eq('candidate_id', user.id),
                supabase.from('resume_language').select('*').eq('candidate_id', user.id),
                supabase.from('resume_certification').select('*').eq('candidate_id', user.id),
                supabase.from('resume_reference').select('*').eq('candidate_id', user.id)
            ]);
            ...
            alert("Önizleme verileri alınırken bir hata oluştu.");
        } finally {
            setLoadingPreview(false);
        }
    }
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            ...
            const { data: { user } } = await supabase.auth.getUser();
            const isMock = candidate.id === 'mock-user';
            ...
            if (avatarFile && user) {
                const url = await handleAvatarUpload(user.id);
                if (url) {
                    avatarUrl = url;
                } else {
                    setSaving(false);
    ...
    async function handleReportUpload(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        try {
            ...
            const fileName = `${user.id}/disability_report_${Date.now()}.${fileExt}`;
            ...
            alert('Rapor başarıyla yüklendi ve Dosyalarım sayfasına eklendi.');
        ...
        <div className="space-y-8 relative">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b pb-4">
                <div className="flex items-center gap-2">
                    <h1 className="text-3xl font-bold tracking-tight text-slate-900">Profilim</h1>
                    {candidate.is_disabled && <span className="text-2xl" title="Engelli Raporu Var">♿</span>}
                    <p className="text-slate-500">Tüm kişisel ve mesleki bilgilerinizi buradan yönetebilirsiniz.</p>
                </div>
                <div className="flex items-center gap-2">
                    <div className="flex items-center gap-2">
