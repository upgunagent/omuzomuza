...
export default function AdminCandidateReportPage() {
    ...
    function exportToExcel() {
        ...
        const infoValueStyle = {
            font: { bold: true, color: { rgb: "4472C4" } }, // Blue Color
            alignment: { vertical: "center", horizontal: "left" }
        };
        const headerStyle = {
            font: { bold: true },
        ...
        const dataStyle = {
            border: borders,
            alignment: { vertical: "center", horizontal: "center", wrapText: false } // Centered
        };
        const wrapDataStyle = {
            border: borders,
            alignment: { vertical: "center", horizontal: "center", wrapText: true } // Centered
        };
        ...
        for (let R = range.s.r; R <= range.e.r; ++R) {
            for (let C = range.s.c; C <= range.e.c; ++C) {
                ...
                if (R === 0) {
                    ...
                    if (C === 0) ws[cell_ref].s = labelStyle;
                    else if (C === 1 || C === 5) ws[cell_ref].s = infoValueStyle;
                } else if (R === 3) {
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    <Link href={`/admin/positions/${id}`}>
                        <Button variant="outline" size="icon"><ArrowLeft className="w-4 h-4" /></Button>
                    </Link>
                    <div>
                        <h1 className="text-2xl font-bold text-slate-800">Aday Takip Raporu (Admin)</h1>
                                    ...
                                    <tr key={candidate.id} className="bg-white hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {/* Edit */}
                                                <Link href={`/admin/positions/${id}/candidates/${candidate.id}/edit`}>
