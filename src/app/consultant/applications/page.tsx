...
export default function ConsultantApplicationsPage() {
    ...
    useEffect(() => {
        ...
        const fetchDataAndSubscribe = async () => {
            ...
            subscription = supabase
                .channel('public:applications:all')
                .on(
                    'postgres_changes',
                    {
                        event: 'UPDATE',
                    ...
                    (payload) => {
                        ...
                        fetchApps(false); // Refetch without full loading spinner if desired, or just silent update
                    }
                )
                .subscribe();
            setLoading(false);
    ...
    async function handleStatusChange(appId: number, newStatus: string) {
        setUpdatingId(appId);
        try {
            // Use RPC to bypass RLS (using v2 which handles UUIDs)
            ...
            // State will likely be updated by Realtime, but optimistic update is good UX
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-3xl font-bold tracking-tight text-slate-900">Başvuru Yönetimi</h1>
                <p className="text-slate-500">Tüm ilanlara yapılan başvuruları buradan yönetebilirsiniz.</p>
            </div>
                        ...
                        <CardContent className="p-5 flex flex-col md:flex-row justify-between items-center gap-4">
                            <div className="flex flex-col gap-1">
                                ...
                                <div className="text-sm text-slate-500 flex items-center gap-1">
                                    Başvuru: <span className="font-medium text-slate-900">{app.jobs.title}</span>
